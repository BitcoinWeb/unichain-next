...
const NS_UNICHAIN_INITIATOR = slab.subarray(32, 64)
const NS_UNICHAIN_RESPONDER = slab.subarray(64, 96)

sodium.crypto_generichash(NS, b4a.from('unichain2'))
sodium.crypto_generichash(NS_UNICHAIN_INITIATOR, b4a.from([0]), NS)
sodium.crypto_generichash(NS_UNICHAIN_RESPONDER, b4a.from([1]), NS)
...
class Peer {
  ...
  _sendInfo () {
    this.info({
      length: this.handlers.chain.tree.length,
      fork: this.handlers.chain.tree.fork,
  ...
  registerExtension (name, handlers) {
    ...
    const chainExt = new ChainExtension(ext, this, name, handlers)
    ext.peers.add(chainExt)
    this.extensions.set(name, chainExt)
    return chainExt
  }
...
module.exports = class Protocol {
  ...
  registerPeer (key, discoveryKey, handlers = {}, state = null) {
    ...
    this._announceChain(peer.alias, key, discoveryKey)
    return peer
  }
  ...
  uncork () {
    ...
    while (this._pendingCaps.length > 0) {
      ...
      unichainCapability(this.noiseStream.isInitiator, this.noiseStream.handshakeHash, key, cap)
    }
  ...
  _announceChain (alias, key, discoveryKey) {
  ...
  _decodeMessage (type, state) {
    switch (type) {
      ...
      case 2: return this._onchain(messages.chain.decode(state))
      case 3: return this._onunknownchain(messages.unknownChain.decode(state))
    }
  ...
  _onchain (m) {
    ...
    if (peer) {
      const expectedCap = unichainCapability(!this.noiseStream.isInitiator, this.noiseStream.handshakeHash, peer.key)
      ...
      if (peer.resend) this._announceChain(peer.alias, peer.key, peer.discoveryKey)
      this._catch(peer.handlers.onchain(m, peer))
      return
    }
    ...
    function next () {
      if (self._peers.has(hex)) return self._onchain(m)
      self.send(3, messages.unknownChain, -1, { discoveryKey: m.discoveryKey })
    }
  }
  ...
  _onunknownchain (m) {
    ...
    peer.resend = true
    this._catch(peer.handlers.onunknownchain(m, peer))
  }
...
function unichainCapability (initiator, handshakeHash, key, cap = b4a.alloc(32)) {
  const ns = initiator ? NS_UNICHAIN_INITIATOR : NS_UNICHAIN_RESPONDER
